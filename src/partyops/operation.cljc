(ns partyops.operation
  "OperationActor -- one political-organization operation = one
  supervised actor run, expressed as a langgraph-clj StateGraph. The
  advisor (PartyOps-LLM) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the Political Organization
  Governance Governor (:govern) and the rollout phase gate (:decide)
  ...
  One graph run = one political-organization operation (intake ->
  ...
  governor is clean -- see `partyops.phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [partyops.partyopsllm :as partyopsllm]
...
(defn build
  ...
  `partyops.store/Store`).
  opts:
    :advisor      -- a `partyops.partyopsllm/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (partyopsllm/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; PartyOps-LLM inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (partyopsllm/-advise advisor store request)]
            {:proposal p :audit [(partyopsllm/trace request p)]})))

      ;; Political Organization Governance Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
