(ns partyops.governor-contract-test
  "The governor contract as executable tests -- the political-
  organization analog of `cloud-itonami-isic-6512`'s `casualty.
  governor-contract-test`. The single invariant under test:

    PartyOps-LLM never publishes a position the Political Organization
...
(defn- verify!
  "Walks `subject` through verify -> approve, leaving a campaign-
  finance-disclosure evidence checklist on file. Uses distinct
...
(deftest clean-intake-auto-commits
  (let [[db actor] (fresh)
    ...
    (is (= "climate-policy-platform" (:position-name (store/position db "position-1"))) "SSoT actually updated")
...
(deftest member-consensus-share-insufficient-is-held
  (testing "a position whose own recorded member-consensus vote share falls below its own recorded required threshold -> HOLD"
    (let [[db actor] (fresh)
          _ (verify! actor "t5pre" "position-3")
          res (exec-op actor "t5" {:op :actuation/publish-position :subject "position-3"} operator)]
      (is (= :hold (get-in res [:state :disposition])))
      (is (some #{:member-consensus-share-insufficient} (-> (store/ledger db) last :basis)))
      (is (empty? (store/publication-history db))))))

(deftest campaign-finance-disclaimer-missing-is-held-and-unoverridable
  (testing "a missing campaign-finance disclaimer on a position -> HOLD, and never reaches request-approval -- exercised via :disclaimer/screen DIRECTLY, not via the actuation op against an unscreened position (see this actor's governor ns docstring / parksafety's ADR-2607071922 Decision 5 / eldercare's, museum's, conservation's, salon's, entertainment's, casework's, hospital's, facility's, school's, association's, leasing's, behavioral's, secondary's, card's, water's, telecom's, aerospace's, recovery's, consulting's, union's, congregation's, fab's, energy's, care's, navigator's, learning's, banking's, advertising's, polling's, research's, design's, nursing's, sports's, alliedhealth's, laundry's, holdco's, photo's, personalservice's, edsupport's, headoffice's, residential's, cultural's, reserve's, proserv's, sportsevent's, recreation's and sportsclub's ADR-0001s)"
    (let [[db actor] (fresh)
          res (exec-op actor "t6" {:op :disclaimer/screen :subject "position-4"} operator)]
      (is (= :hold (get-in res [:state :disposition])) "settles immediately, no interrupt")
      (is (not= :interrupted (:status res)))
      (is (some #{:campaign-finance-disclaimer-missing} (-> (store/ledger db) first :basis)))
      (is (nil? (store/disclaimer-screen-of db "position-4")) "no clearance written"))))
